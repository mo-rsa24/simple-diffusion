...
def train(cfg: Box, dirs: Dict, model: ComposableUnet, ema: EMA, train_loader: DataLoader, logger, device,  loss_mask_type: str, writer = None, wandb_run = None):
    model.train()
    ...
    log_training_start(
        logger,
        model_name=f"Disentangled Denoising Diffusion Probabilistic Model (Expert: {loss_mask_type or 'None'})",
        ...
        for epoch in range(start_epoch, cfg.training.epochs + 1):
            ...
            for step, train_batch in enumerate(train_loader, 1):
                ...
                if cfg.diffusion.loss_type == 'l1':
                    raw_loss = F.l1_loss(noise, pred_noise)
                elif cfg.diffusion.loss_type == 'l2':
                    raw_loss = F.mse_loss(noise, pred_noise, reduction='none')  # Get per-pixel loss
                elif cfg.diffusion.loss_type == "huber":
                    raw_loss = F.smooth_l1_loss(noise, pred_noise)

                if loss_mask_type == 'digit':
                    mask = train_batch['digit_mask'].to(device)
                    loss = (raw_loss * mask).sum() / mask.sum()  # Average loss over masked pixels
                elif loss_mask_type == 'bbox':
                    mask = train_batch['bbox_mask'].to(device)
                    loss = (raw_loss * mask).sum() / mask.sum()  # Average loss over masked pixels
                else:
                    loss = raw_loss.mean()  # Default behavior: average over all pixels
                # --- END MASKED LOSS ---
