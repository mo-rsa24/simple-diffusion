...
def train(cfg, dirs: Dict, model, ema: EMA, train_loader: DataLoader, logger, device, writer=None, wandb_run=None):
    ...
    log_training_start(
        logger,
        model_name="ClassifierGuidedUNet",
    ...
    try:
        for epoch in range(start_epoch, cfg.training.epochs + 1):
            ...
            for step, batch in enumerate(train_loader, 1):
                ...
                labels = batch.get("digit_label", None)
            ...
            ema.apply_shadow()
            generated = generate_batch(lambda x_, t_: model(x_, t_)[0] if isinstance(model(x_, t_), tuple) else model(x_, t_),
