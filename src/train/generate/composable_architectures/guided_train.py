    ...
    try:
        for epoch in range(start_epoch, cfg.training.epochs + 1):
            ...
            for step, batch in enumerate(train_loader, 1):
                ...
                conds = torch.stack([
                    batch.get("digit_label", torch.zeros(bsz)).to(device),
                    batch.get("color_label", torch.zeros(bsz)).to(device),
                    batch.get("bbox_label", torch.zeros(bsz)).to(device)
                ], dim=1).float()
